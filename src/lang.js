...
window.point = (x, y) => window.frect(x, y, 2, 2)
window.arc = (...args) => ctx.arc(...args)
window.arcc = (...args) => ctx.arcTo(...args)
window.triang = (x1, y1, x2, y2, x3, y3) => {
  window.begin()
  window.line(x1, y1)
  window.line(x2, y2)
  window.line(x3, y3)
...
window.quad = (x1, y1, x2, y2, x3, y3, x4, y4) => {
  window.begin()
  window.line(x1, y1)
  window.line(x2, y2)
  window.line(x3, y3)
...
window.ellips = (x, y, w, h, r) => {
  window.begin()
  ctx.ellipse(x, y, w, h, r ? (r * PI) / 180 : 0, 0, TWO_PI)
}
window.sellips = (...args) => {
  window.ellips(...args)
  window.stroke()
...
window.clear = () => ctx.clearRect(0, 0, window.w, window.h)
window.clearf = (arg) => (clearFrame = arg)
window.bground = (arg) => {
  window.push()
  window.fstyle(arg)
  window.frect(0, 0, window.w, window.h)
  window.pop()
}
...
window.lgrad = (...args) => {
  const g = ctx.createLinearGradient(...args)
  g.stop = (...args) => {
    g.addColorStop(...args)
    return g
  }
...
window.rgrad = (...args) => {
  const g = ctx.createRadialGradient(...args)
  g.stop = (...args) => {
    g.addColorStop(...args)
    return g
  }
