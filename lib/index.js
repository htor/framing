...
function save (cm) {
  ...
  let [setup, loop] = js.split(/\n\s*?\/\/\s*?\bloop\b\s*?\n/)
  window.setupCode = loop && setup
  window.loopCode = loop || setup
  console.log(loop ? 'setup+loop' : 'loop only');
  setQueryParam('id', btoa(encodeURIComponent(js)))
  setFavicon()
}
